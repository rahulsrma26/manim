...
class Ninja:
    ...
    def __init__(self, emotion='happy', leader=False):
        ...
        for obj in self.svg.submobjects:
            obj.set_stroke(color=WHITE, width=0).set_fill(color=RED, opacity=0)

        for part, indexes in Ninja.BODY_MAP.items():
            setattr(self, part, [self.svg.submobjects[i] for i in indexes])
            for idx in indexes:
                color = (Ninja.LEADER_MAP if leader else Ninja.COLOR_MAP)[part]
                self.svg.submobjects[idx].set_fill(color=color, opacity=1)

        if emotion == 'confused':
            self.eyes[0].add_updater(lambda m, dt: m.rotate(-3*dt))
            self.eyes[1].add_updater(lambda m, dt: m.rotate(3*dt))
        ...
        #             blink.time += dt
        #             if 3.5 < blink.time < 4:
        #                 m.set_height(max(h*math.sqrt(abs(blink.time-3.75)), h*0.2))
        #             elif blink.time > 4:
        #                 m.set_height(h)
        #                 blink.time = 0
        #         blink.time = 0
